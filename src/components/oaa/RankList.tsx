...
function SortableRow({ item, rank }: { item: RankItem; rank: number }) {
  ...
  return (
    ...
    >
      <span className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-brand-500 text-white text-[11px] tracking-[0.04em]">
        {rank}
      </span>
      <div className="flex-1 min-w-0">
        <div className="text-[15px] leading-[1.5] font-medium text-foreground truncate">
          {item.title}
        </div>
        {item.description && (
          <div className="text-[13px] leading-[1.45] text-muted-foreground truncate">
            {item.description}
          </div>
        )}
      </div>
